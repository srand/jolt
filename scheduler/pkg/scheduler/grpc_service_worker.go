...
func (s *workerService) GetTasks(stream protocol.Worker_GetTasksServer) error {
	...
	executor, err := s.scheduler.NewExecutor(status.WorkerId, status.BuildId)
	if err != nil {
		log.Errorf("Executor failed to enlist for build: %s - %v", status.BuildId, err)
		return err
	}
	defer executor.Close()

	log.Infof("Executor enlisted for build: %s", status.BuildId)
	...
	for {
		select {
		case <-executor.Done():
			log.Infof("Executor delisted for build: %s", status.BuildId)
			return nil
